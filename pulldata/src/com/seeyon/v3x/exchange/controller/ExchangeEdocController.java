...
import com.seeyon.v3x.exchange.domain.*;
import com.seeyon.v3x.exchange.enums.EdocExchangeMode.EdocExchangeModeEnum;
import com.seeyon.v3x.exchange.manager.*;
...
public class ExchangeEdocController extends BaseController {
    ...
    public ModelAndView change(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (null != modelType && !"".equals(modelType) && "toSend".equals(modelType)) {
            ...
            edocSummaryZ.setSendTo(sendedNames);
            edocSummaryZ.setSendToId(typeAndIds);
            edocManager.update(edocSummaryZ);
            //结束
            //[客开][徐矿集团]--功能：当做公文交换时点击发送公文，添加监听事件 作者：zhou 时间：20201221
            EdocSummary summaryKk = edocManager.getEdocSummaryById(Long.parseLong(summaryIdz), true);
            EdocExchangeSendEvent sendEvent = new EdocExchangeSendEvent(this);
            sendEvent.setEdocSummary(summaryKk);
            sendEvent.setSummaryId(Long.parseLong(summaryIdz));
            EventDispatcher.fireEvent(sendEvent);
            //[客开][徐矿集团]--功能：当做公文交换时点击发送公文，添加监听事件 作者：zhou 时间：20201221
...
//                List<MessageReceiveVo> msgs = new ArrayList<MessageReceiveVo>();
            ...
            try {
                ...
                //当G6版本关闭登记开关时，那么签收时，还要生成登记数据，并生成待分发affair
                    ...
                    LinkOpenType linkOpenType = LinkOpenType.href;
...
//		sendMessageToRegister(user, agentToId, reAffair);//待登记人不需要消息了
        ...
        register.setCreateTime(new Timestamp(new Date().getTime()));
        register.setUpdateTime(new Timestamp(new Date().getTime()));
        ...
        */
        ...
        if (summary != null && record != null) {
            ...
            registerBody.setCreateTime(new Timestamp(new Date().getTime()));
            register.setRegisterBody(registerBody);
        }
...
//        	record.setRegisterTime(new java.sql.Timestamp(register.getRegisterDate().getTime()));
        ...
        LinkOpenType linkOpenType = LinkOpenType.open;
        ...
        linkOpenType = LinkOpenType.href;
        String agentApp = "agent";
    ...
    private void sendRegisterMessage(String key, String subject, String userName, String agentName, long fromUserId, long registerId,
                                     long toUserId, String colAgent, String agentDeal, String url, LinkOpenType linkType,
...
//			userMessageManager.sendSystemMessage(messageContent, ApplicationCategoryEnum.edocRec, fromUserId, receiver);
    ...
    public ModelAndView edit(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if ("sent".equals(modelType)) {
            ...
            mav.addObject("bean", bean);
            //[徐矿集团]在这里添加获取发文类型返回到页面上显示，zhou:2021-01-23 16:24 开始
            String sql = "select list3 from EDOC_SUMMARY_EXTEND where summary_id=" + summary.getId();
            try (JDBCAgent jdbcAgent = new JDBCAgent();) {
                jdbcAgent.execute(sql);
                List<Map<String, Object>> mapList = jdbcAgent.resultSetToList();
                if (mapList.size() > 0) {
                    Map<String, Object> map = mapList.get(0);
                    String list3 = map.get("list3") + "";
                    if ("0".equals(list3)) {
                        mav.addObject("list3", "党委发文");
                    } else if ("0".equals(list3)) {
                        mav.addObject("list3", "行政发文");
                    } else {
                        mav.addObject("list3", "");
                    }
                }
            } catch (Exception e) {
                logger.error("zhou:ExchangeEdocController代码中的2312行代码出现错误：" + e.getMessage());
            }
            //[徐矿集团]在这里添加获取发文类型返回到页面上显示，zhou:2021-01-23 16:24 截止
            mav.addObject("copies", copies);
            //获取签收单位或者签收部门所在的单位。
            Long exchangeAccountId = getAccountIdOfSendByOrgIdAndOrgType(bean.getExchangeOrgId(), bean.getExchangeType());
            mav.addObject("allowShowEdocInSend", EdocSwitchHelper.allowShowEdocInSend(exchangeAccountId));
